      ...
      <div className="h-full overflow-y-auto bg-[#fcfcfc] dark:bg-[#111]">
        ...
        <div className="bg-gradient-to-r from-white to-[#f1f5fd] dark:from-[#141414] dark:to-[#1a2035] px-[80px] py-[52px] flex items-center gap-[60px]">
          <div className="flex-1 flex flex-col gap-[16px]">
            {/* Tag */}
            <motion.div {...springEnter(0)} className="flex items-center gap-[4px] self-start bg-[#e6f0ff] dark:bg-[#0067ff]/10 border border-[#91beff] dark:border-[#0067ff]/30 rounded-[8px] px-[8px] py-[6px]">
            ...
            {/* Subtext */}
            <motion.p {...springEnter(0.12)} className="font-normal text-[16px] text-[#595959] dark:text-[#8c8c8c] tracking-[-0.18px] leading-[26px] max-w-[480px]">
              Karamcharis are AI employees that categorize messages, collect leads, sync data, and automate your WhatsApp operations 24/7.
            </motion.p>

            {/* CTA buttons */}
            <motion.div {...springEnter(0.17)} className="flex items-center gap-[12px] pt-[4px]">
              <button
                onClick={() => {
          ...
          {/* Hero illustration */}
          <motion.div {...springEnter(0.1)} className="flex-shrink-0 w-[366px] h-[366px]">
            <Image
        ...
        <div id="choose-karamchari" className="px-[80px] py-[60px] flex flex-col gap-[20px]">
          <motion.p {...springEnter(0)} className="font-semibold text-[18px] text-[#262626] dark:text-white tracking-[-0.3px]">
            Choose an AI Karamchari
          </motion.p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[20px]">
            {state.agents.map((agent, i) => (
              <motion.div key={agent.key} {...springEnter(0.08 + i * 0.07)}>
                <AgentCard
                  agent={agent}
                  state={state.states[agent.key]}
                  onDeploy={(e) => {
                    const card = e.currentTarget.closest(".agent-card-container");
                    if (card) {
                      setTriggerRect(card.getBoundingClientRect());
                    }
                    state.setOpenModal(agent.key);
                  }}
                  onConfigure={(e) => {
                    const card = e.currentTarget.closest(".agent-card-container");
                    if (card) {
        ...
        <div className="px-[80px] py-[40px]">
          <motion.div {...springEnter(0)} className="bg-[#ebf6fd] dark:bg-[#0a1a2e] rounded-[18px] p-[24px] flex flex-col gap-[24px]">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-[12px]">
              {WHY_ITEMS.map((item, i) => (
                <motion.div key={item.title} {...springEnter(0.06 + i * 0.06)} className="bg-white dark:bg-[#1a1a1a] rounded-[12px] p-[16px] flex items-center gap-[12px]">
