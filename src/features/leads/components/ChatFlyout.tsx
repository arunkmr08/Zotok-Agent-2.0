...
export function ChatFlyout({ lead, onClose }: Props) {
  const msgs = [
    { from: "them", text: `Hi, I'm ${lead.name} from ${lead.location}.` },
    { from: "me",   text: "Hello! How can I help you today?" },
    { from: "them", text: lead.summary },
    { from: "me",   text: "Got it. Let me check what we can do for you." },
    { from: "them", text: "Great, looking forward to it!" },
  ];
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-[380px] bg-white dark:bg-[#1a1a1a] border-l border-black/[0.08] dark:border-white/[0.06] flex flex-col z-40 shadow-2xl">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-black/[0.06] dark:border-white/[0.06]">
        <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0" style={{ backgroundColor: lead.color }}>
          {lead.avatar}
        </div>
          ...
          <div className="relative flex-1">
            ...
            <Tooltip>
              <TooltipTrigger render={
                <button
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-[26px] px-[6px] flex items-center gap-[3px] rounded-md transition-all duration-200 hover:opacity-90"
