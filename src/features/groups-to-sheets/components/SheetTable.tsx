...
type Props = Pick<GroupsToSheetsState, "sheet">;
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-black/[0.08] dark:border-white/[0.06] bg-white dark:bg-[#1a1a1a] flex-shrink-0">
        <div>
          ...
          <p className="text-sm text-[#6d6c6b]">{sheet.rows.length} rows</p>
        </div>
        ...
        <table className="w-full text-sm min-w-[700px]">
          ...
          <tbody className="divide-y divide-black/[0.06] dark:divide-white/[0.06]">
            {sheet.rows.map((row, i) => (
              <tr key={i} className="bg-white dark:bg-[#1a1a1a] hover:bg-[#f4f3ef] dark:hover:bg-[#2a2a2a] transition-colors">
                <td className="px-4 py-3 text-[#858481] text-xs">{i + 1}</td>
                {VISIBLE_COLS.map((col) => (
