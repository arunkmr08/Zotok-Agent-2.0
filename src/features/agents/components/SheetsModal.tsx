        ...
        <div className="bg-[#f8f8f7] dark:bg-[#1a1a1a] border border-black/[0.12] dark:border-white/[0.08] rounded-[18px] drop-shadow-[0px_8px_16px_rgba(0,0,0,0.06)] w-full max-w-[720px] flex flex-col gap-[20px] p-[21px]">
          ...
          {(view === "connect" || view === "loading") && (
            <>
              ...
              <div className="flex gap-[12px]">
                <button onClick={handleClose} className="flex-1 bg-white dark:bg-[#262626] border border-[#e8e6e0] dark:border-white/[0.1] rounded-[8px] py-[9px] font-semibold text-[14px] text-[#34322d] dark:text-[#d9d9d9] tracking-[-0.09px] leading-[18px] text-center hover:bg-[#f4f3ef] dark:hover:bg-[#303030] transition-colors">
                  Cancel
                </button>
                <button onClick={handleConnect} disabled={view === "loading"} className="flex-1 bg-[#0067ff] hover:bg-[#0055d4] disabled:opacity-70 transition-colors rounded-[8px] py-[9px] font-semibold text-[14px] text-white tracking-[-0.09px] leading-[18px] text-center flex items-center justify-center gap-[8px]">
            ...
            <>
              ...
              <div className="bg-white dark:bg-[#262626] border border-black/[0.06] dark:border-white/[0.06] rounded-[12px] flex flex-col gap-[24px] px-[20px] py-[16px]">
                ...
                <div className="grid grid-cols-3 gap-[16px] max-h-[420px] overflow-y-auto pr-1">
                  {/* Blank */}
                  <button onClick={() => setSelectedSheet("blank")} className={cn("flex flex-col items-center justify-center gap-[8px] rounded-[8px] border-dashed border-2 py-[40px] transition-colors", selectedSheet === "blank" ? "border-[#0067ff] bg-[#e6f0ff] dark:bg-[#0f2040]" : "border-black/[0.12] dark:border-white/[0.12] bg-[#f8f8f7] dark:bg-[#1f1f1f] hover:border-black/[0.2]")}>
                  ...
                  {SHEET_OPTIONS.filter(s => s.id !== "blank").map((s) => (
                    <button key={s.id} onClick={() => setSelectedSheet(s.id)} className={cn("flex flex-col items-start rounded-[8px] border overflow-hidden transition-all", selectedSheet === s.id ? "border-[#0067ff] shadow-[0px_8px_32px_rgba(0,103,255,0.15)]" : "border-black/[0.12] dark:border-white/[0.1] shadow-[0px_8px_32px_rgba(0,0,0,0.06)] hover:border-black/[0.2]")}>
              ...
              <button onClick={() => setView("columns")} disabled={!selectedSheet} className="w-full bg-[#0067ff] hover:bg-[#0055d4] disabled:opacity-40 disabled:cursor-not-allowed transition-colors rounded-[8px] py-[10px] font-semibold text-[14px] text-white tracking-[-0.09px] leading-[18px] text-center">
                Continue
              </button>
            </>
          )}
          ...
          {view === "columns" && (
            <>
              ...
              <div className="flex items-center justify-between h-[36px]">
                ...
                <button onClick={() => setNewCol("")} className="flex items-center gap-[6px] bg-white dark:bg-[#262626] border border-[#e8e6e0] dark:border-white/[0.1] pl-[13px] pr-[15px] py-[9px] rounded-[8px] font-semibold text-[14px] text-[#34322d] dark:text-[#d9d9d9] tracking-[-0.09px] leading-[18px] hover:bg-[#f4f3ef] dark:hover:bg-[#303030] transition-colors">
              ...
              <div className="bg-white dark:bg-[#262626] border border-black/[0.06] dark:border-white/[0.06] rounded-[8px] py-[8px] overflow-hidden max-h-[300px] overflow-y-auto">
                {columns.map((col, i) => (
                  <div
                    ...
                    className={cn("flex items-center gap-[10px] px-[12px] py-[12px] border-b border-black/[0.06] dark:border-white/[0.06] transition-colors", dragOver === i ? "bg-[#e6f0ff] dark:bg-[#0f2040]" : "hover:bg-black/[0.02] dark:hover:bg-white/[0.02]")}
                  >
                    ...
                    <Tooltip>
                      <TooltipTrigger render={
                        <button onClick={() => setColumns((p) => p.filter((_, j) => j !== i))} className="w-[18px] h-[18px] flex items-center justify-center text-[#c0bfbd] dark:text-[#595959] hover:text-red-500 transition-colors flex-shrink-0">
              ...
              <div className="flex justify-end">
                <button
                  onClick={() => { if (newCol.trim()) setColumns((p) => [...p, { name: newCol.trim() }]); setNewCol(""); setView("success"); }}
          ...
          {view === "success" && (
            <div className="flex flex-col items-center gap-[16px] py-[24px] text-center">
              ...
              <button onClick={handleDone} className="w-full bg-[#0067ff] hover:bg-[#0055d4] transition-colors rounded-[8px] py-[10px] font-semibold text-[14px] text-white tracking-[-0.09px] leading-[18px] text-center">
                Done
              </button>
            </div>
          )}
