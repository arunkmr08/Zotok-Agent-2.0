...
import { HISTORY_OPTIONS } from "@/features/whatsapp/constants";
  ...
  return (
    <Dialog open={historyModal} onOpenChange={setHistoryModal}>
      <DialogContent className="max-w-sm">
        ...
        <div className="flex items-center gap-2 text-sm text-green-800 dark:text-green-300 bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800 rounded-lg px-3 py-2 mb-3">
          ...
          {HISTORY_OPTIONS.map((opt) => (
            <div
              ...
              className={cn(
                ...
                selectedHistory === opt.days ? "border-blue-500 bg-blue-50 dark:bg-blue-950" : "border-black/[0.08] dark:border-white/[0.08] hover:border-black/[0.12]"
              )}
            >
              <span className="text-sm text-[#34322d] dark:text-[#dadada]">{opt.label}</span>
              <div className={cn("w-4 h-4 rounded-full border-2 transition-colors", selectedHistory === opt.days ? "border-blue-500 bg-blue-500" : "border-black/[0.12]")} />
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2 mt-1">
          ...
          <Button className="flex-1" onClick={handleHistoryContinue}>Continue</Button>
        </div>
      </DialogContent>
    </Dialog>
  );
